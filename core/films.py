...
DUO = {
    ...
    "noir": (
        ...
        "{costar} — two detectives who are ALREADY WEARING plain worn trench coats AND "
        "fedora hats from the very first frame; the hats sit firmly on their heads for "
        "the entire video, through all the running — clothing and hats never appear, "
        "vanish or change — plus ONE fleeing figure seen only briefly and only from "
        "behind. The two detectives study a small "
    ...
    "drama": (
        ...
        "window light. EXACTLY THREE people appear in this entire video and NEVER "
        "anyone else — no waiter, no passers-by, no background people at all: (a) the "
        "person in Image 1; (b) a young {costar} — the person in Image 1's PARTNER, "
